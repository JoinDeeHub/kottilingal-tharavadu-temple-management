import { useRef, Suspense, lazy, Component } from 'react'
...
function DepthCard({ icon, title, desc, delay = 0, href }) {
  const content = (
    ...
    >
      ...
      <h3 style={{ color: 'var(--gold)', fontFamily: 'Cinzel', letterSpacing: '0.08em', marginBottom: '0.5rem', fontSize: '0.95rem' }}>{title}</h3>
      <p style={{ color: 'var(--ivory-dim)', fontSize: '0.95rem', lineHeight: 1.7 }}>{desc}</p>
    </motion.div>
  )
  return href ? <Link to={href} style={{ display: 'block', height: '100%' }}>{content}</Link> : content
}
    ...
    <div style={{ background: 'var(--navy)', minHeight: '100vh', position: 'relative' }}>
      ...
      <section ref={heroRef} style={{
        ...
        <motion.div style={{ y: bgY, position: 'absolute', inset: 0, zIndex: 0 }}>
          <div style={{
            ...
            background: `
              ...
              radial-gradient(ellipse 100% 80% at 50% 100%, rgba(8,12,23,0.95) 0%, transparent 60%),
              linear-gradient(180deg, #080C17 0%, #0D1322 40%, #111828 70%, #080C17 100%)
            `,
          }} />
        </motion.div>
        ...
        <motion.div style={{
          ...
          <span style={{ fontFamily: 'Cinzel', fontSize: 'clamp(18rem,40vw,40rem)', color: 'rgba(201,168,76,0.07)', lineHeight: 1 }}>ॐ</span>
        </motion.div>

        {/* Incense sticks */}
        <div style={{ position: 'absolute', bottom: '8%', left: '50%', marginLeft: '-210px', zIndex: 2 }}>
          <Suspense fallback={null}><ErrBound><IncenseSmoke /></ErrBound></Suspense>
        </div>
        <div style={{ position: 'absolute', bottom: '8%', left: '50%', marginLeft: '90px', zIndex: 2 }}>
          <Suspense fallback={null}><ErrBound><IncenseSmoke flipped /></ErrBound></Suspense>
        </div>
        ...
        <motion.div style={{
          ...
          zIndex: 10, textAlign: 'center', padding: '0 1.5rem', position: 'relative',
        }}>
          {/* Temple image circle */}
          <motion.div
            animate={{ boxShadow: [
          ...
          >
            <div style={{
              ...
              overflow: 'hidden',
              background: 'radial-gradient(circle, rgba(139,0,0,0.3) 0%, rgba(8,12,23,0.9) 70%)',
              boxShadow: 'inset 0 0 40px rgba(201,168,76,0.08)',
            }}>
              <img
                src="/temple1.jpg"
                alt="Kottilingal Bhagavathi Temple"
                style={{ width: '100%', height: '100%', objectFit: 'cover', opacity: 0.95 }}
                onError={e => { e.target.style.display = 'none'; e.target.parentElement.innerHTML = '<div style="font-size:4rem;display:flex;align-items:center;justify-content:center;height:100%">🛕</div>' }}
              />
            </div>
          </motion.div>

          <CinematicText text="KOTTILINGAL THARAVADU" tag="h1" color="var(--gold)" delay={200} />
          <motion.h2
          ...
          <motion.div
            initial={{ opacity: 0, y: 30 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 2.2 }}
            ...
            <Link to="/donate">
              <motion.button
                ...
                style={{
                  ...
                  boxShadow: '0 0 20px rgba(139,0,0,0.4)', cursor: 'pointer',
                }}
              >🪔 MAKE AN OFFERING</motion.button>
            </Link>
            <Link to="/about">
              <motion.button
                whileHover={{ scale: 1.06, background: 'rgba(201,168,76,0.1)' }}
                whileTap={{ scale: 0.96 }}
                style={{
      ...
      <section style={{ padding: 'clamp(4rem,8vw,8rem) 1.5rem', maxWidth: 1100, margin: '0 auto' }}>
        <ScrollReveal>
          <div style={{ textAlign: 'center', marginBottom: '4rem' }}>
            ...
            <CinematicText text="A LIVING SACRED TRADITION" tag="h2" color="var(--ivory)" delay={0} />
      ...
      <section style={{ position: 'relative', overflow: 'hidden', padding: 'clamp(5rem,10vw,10rem) 1.5rem' }}>
        <ScrollParallax speed={0.15} direction="up" style={{ position: 'absolute', inset: 0, zIndex: 0 }}>
          <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse 80% 60% at 50% 50%, rgba(139,0,0,0.18) 0%, transparent 70%)' }} />
        </ScrollParallax>
        <ScrollReveal>
          ...
          <Link to="/gallery">
            <motion.button
              ...
              style={{
                ...
                letterSpacing: '0.25em', fontSize: '0.75rem', cursor: 'pointer', transition: 'all 0.4s',
              }}
      ...
      {/* ── VIDEO TEASER ────────────────────────────── */}
      <section style={{ padding: 'clamp(2rem,5vw,5rem) 1.5rem', maxWidth: 800, margin: '0 auto', textAlign: 'center' }}>
        <ScrollReveal>
          <p style={{ color: 'var(--gold-dim)', letterSpacing: '0.4em', fontSize: '0.7rem', fontFamily: 'Cinzel', marginBottom: '0.75rem' }}>TEMPLE IN MOTION</p>
          <CinematicText text="SACRED VIDEOS" tag="h2" color="var(--ivory)" />
          <p style={{ color: 'var(--ivory-dim)', margin: '1rem auto 2rem', maxWidth: '40ch', lineHeight: 1.8 }}>Experience the live rituals, festivals and sacred moments of the temple.</p>
          <Link to="/videos">
            <motion.button
              ...
              style={{
                ...
                color: 'var(--gold)', fontFamily: 'Cinzel',
                padding: '0.9rem 2.5rem', borderRadius: 9999,
                letterSpacing: '0.15em', fontSize: '0.85rem', cursor: 'pointer',
              }}
            >▶ WATCH TEMPLE VIDEOS</motion.button>
          </Link>
        </ScrollReveal>
      </section>
